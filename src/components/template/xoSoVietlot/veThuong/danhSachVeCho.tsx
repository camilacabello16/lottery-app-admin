...
const DanhSachVeCho = ({ route }: any) => {
    ...
    const getData = () => {
        fetch(rootApi.rootApi + ORDER_DETAIL + "?gameId=3&status=2&staff_id=" + loginData.userData?.staff_id + '&lotteryType=' + (selectedType == 0 ? "" : selectedType), {
            method: 'get',
            headers: {
            ...
            .then((responseJson) => {
                if (responseJson.error == 401) {
                    ...
                    if (responseJson.data) {
                        ...
                        setData(responseJson.data.filter(o => o.lotteryType != 5).reverse());
                    } else {
                        setData([])
                    }
                }
    ...
    const holdOrder = (data) => {
        axios.post(rootApi.rootApi + HOLD_ORDER, {
            ...
            .then(res => {
                console.log("GIUVE", JSON.stringify(res).includes("thành công"));
                if (JSON.stringify(res).includes("thành công")) {
                    alert({
    ...
    return (
        <View w="100%" alignItems="center">
            ...
            <Header headerTitle="VÉ THƯỜNG" subTitle="Danh sách vé chờ" backArrow url='HomeScreen' screen='VIETLOT_VETHUONG' />
            {/* <View style={{
        width: '100%',
        marginVertical: 8
      }}>
